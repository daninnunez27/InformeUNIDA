using System;
using System.Collections.Generic;

#nullable disable

namespace InformeUNIDA.Models
{
    public partial class Ly2Docente1
    {
        public decimal NumFunc { get; set; }
        public string Faculdade { get; set; }
        public string Depto { get; set; }
        public string Ativo { get; set; }
        public string NomeCompl { get; set; }
        public string NomeAbrev { get; set; }
        public string Endereco { get; set; }
        public string EndNum { get; set; }
        public string EndCompl { get; set; }
        public string Bairro { get; set; }
        public string Municipio { get; set; }
        public string Cep { get; set; }
        public string Fone { get; set; }
        public string Cpf { get; set; }
        public string RgTipo { get; set; }
        public string RgNum { get; set; }
        public DateTime? RgDtexp { get; set; }
        public string RgUf { get; set; }
        public string RgEmissor { get; set; }
        public string Endcom { get; set; }
        public string EndcomNum { get; set; }
        public string EndcomCompl { get; set; }
        public string EndcomBairro { get; set; }
        public string EndcomMunicipio { get; set; }
        public string EndcomCep { get; set; }
        public string FoneCom { get; set; }
        public decimal? PercDedicMens { get; set; }
        public string EMail { get; set; }
        public string Mailbox { get; set; }
        public string Titulacao { get; set; }
        public string AtuacaoProfis { get; set; }
        public string Obs { get; set; }
        public string CprofNum { get; set; }
        public string CprofSerie { get; set; }
        public DateTime? CprofDataexp { get; set; }
        public string CprofUf { get; set; }
        public string HabTac { get; set; }
        public string SenhaDol { get; set; }
        public DateTime? DtHabilitDol { get; set; }
        public string Celular { get; set; }
        public string FaxRes { get; set; }
        public string FaxCom { get; set; }
        public string NomeEmpresa { get; set; }
        public string EMailCom { get; set; }
        public decimal? Banco { get; set; }
        public string Agencia { get; set; }
        public string ContaBanco { get; set; }
        public string TipoPessoa { get; set; }
        public string RazaoSocial { get; set; }
        public string Endemp { get; set; }
        public string EndempNum { get; set; }
        public string EndempCompl { get; set; }
        public string EndempBairro { get; set; }
        public string EndempMunicipio { get; set; }
        public string EndempCep { get; set; }
        public string FoneEmp { get; set; }
        public string FaxEmp { get; set; }
        public string EMailEmp { get; set; }
        public string CgcEmp { get; set; }
        public string Categoria { get; set; }
        public string Re { get; set; }
        public DateTime? DtAdmissao { get; set; }
        public DateTime? DtNasc { get; set; }
        public string Sexo { get; set; }
        public string EstCivil { get; set; }
        public DateTime? DtUltTitulo { get; set; }
        public string FecharTurmaInternet { get; set; }
        public string PaisRes { get; set; }
        public string ObsTelRes { get; set; }
        public string ObsTelCom { get; set; }
        public string UrlParticular { get; set; }
        public string UrlProfissional { get; set; }
        public string Pispasep { get; set; }
        public string EndcomPais { get; set; }
        public decimal? Pessoa { get; set; }
        public string ContratoTrabalho { get; set; }
        public string RegimeTrabalho { get; set; }
        public string OutraFaculdade { get; set; }
        public string PrimeiroNome { get; set; }
        public string NomeMeio { get; set; }
        public string Sobrenome { get; set; }
        public string CodLattes { get; set; }
    }
}
