using System;
using System.Collections.Generic;

#nullable disable

namespace InformeUNIDA.Models
{
    public partial class PersonaMig
    {
        public int Cpersona { get; set; }
        public string DpersonaOriginal { get; set; }
        public string ApePaternoOriginal { get; set; }
        public string ApeMaternoOriginal { get; set; }
        public string NombreOriginal { get; set; }
        public string Dpersona { get; set; }
        public string ApePaterno { get; set; }
        public string ApeMaterno { get; set; }
        public string Nombre { get; set; }
        public string Dato01 { get; set; }
        public string Dato03 { get; set; }
        public string Dato05 { get; set; }
        public string Dato07 { get; set; }
        public string Dato09 { get; set; }
        public string Dato11 { get; set; }
        public string Dato13 { get; set; }
        public string Dato15 { get; set; }
        public string Dato17 { get; set; }
        public int? Nnom { get; set; }
        public int? Npat { get; set; }
        public int? CpersonaMin { get; set; }
        public int? Cantida { get; set; }
        public int? Cuadra { get; set; }
        public int? OriginalDoble { get; set; }
    }
}
