...
namespace InformeUNIDA.Controllers
{
    ...
    public class EncuestaDocenteInfraestructuraController : Controller
    {
        public FileResult ExportarExcelReporteEncuestaDocenteInfraestructuraLista(Int32 cinstitucion_p, Int32 cprograma_p, Int32 cperiodo_p, Int32 ccarrera_p, Int32 cprofesor_p)
        {
            var url = $"" + Configuracion.url + "/api/encuestaDocenteInfraestructuraLista/" + cinstitucion_p.ToString() + "/" + cprograma_p.ToString() + "/" + cperiodo_p.ToString() + "/" + ccarrera_p.ToString();
            var request = (HttpWebRequest)WebRequest.Create(url);
            request.Method = "GET";
            request.ContentType = "application/json";
            request.Accept = "application/json";

            List<EncuestaDocenteInfraestructuraDTO> resultado = new List<EncuestaDocenteInfraestructuraDTO>();
                ...
                using (WebResponse response = request.GetResponse())
                {
                    using (Stream strReader = response.GetResponseStream())
                    {
                        ...
                        using (StreamReader objReader = new StreamReader(strReader))
                        {
                            ...
                            List<EncuestaDocenteInfraestructuraDTO> lista = JsonConvert.DeserializeObject<List<EncuestaDocenteInfraestructuraDTO>>(json);

                            string contentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
                            string fileName = "DocenteInfraestructuraLista.xlsx";
                            try
                            {
                                using (var workbook = new XLWorkbook())
                                {
                                    IXLWorksheet worksheet =
                                    workbook.Worksheets.Add("DocenteInfraestructuraLista");
                                    worksheet.Cell(1, 1).Value = "ENCUETA DE DOCENTE A INFRAESTRUCTURA - LISTA";
                                    ...
                                    worksheet.Cell(2, 9).Value = "CANT. DOCENTES";
                                    ...
                                    for (int index = 1; index <= lista.Count; index++)
                                    {
                                        ...
                                        worksheet.Cell(index + 2, 9).Value = lista[index - 1].cant_profesores;
                                    }
